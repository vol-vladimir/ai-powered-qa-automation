...
function qaUser(label: string) {
  const tag = uniqueTag();
  return {
    name: `qa-ds214-${label} ${tag}`,
    email: `qa-ds214-${label}-${tag}@college.edu`,
    password: DEFAULT_PASSWORD,
  };
}

test.describe("Didaxis Studio — add user in Settings (DS-214)", () => {
  ...
  test("TC-001: open Add User dialog and see Name, Email, Password, Role, Create User", async ({
    page,
  }) => {
    const settings = new SettingsPage(page);
  ...
  test("TC-002: successfully create a user with EDITOR role", async ({
    page,
  }) => {
  ...
  test("TC-003: create user with ADMIN role", async ({ page }) => {
  ...
  test("TC-004: create user with VIEWER role", async ({ page }) => {
  ...
  test("TC-005: Create User stays disabled when required fields are empty", async ({
    page,
  }) => {
    const settings = new SettingsPage(page);
    const modal = settings.addUserModal;
  ...
  test("TC-006: Create User stays disabled when only Name is filled", async ({
    page,
  }) => {
    const settings = new SettingsPage(page);
    const modal = settings.addUserModal;
    ...
    await settings.openAddUser();
    await modal.fillName("qa-ds214-incomplete");
    await expect(modal.createUserButton).toBeDisabled();
    await expect(modal.dialog).toBeVisible();
  });

  test("TC-007: closing dialog without submit does not add a user", async ({
    page,
  }) => {
  ...
  test("TC-008: create user with plus-tag style email local part", async ({
    page,
  }) => {
    const tag = uniqueTag();
    const name = `qa-ds214-plus ${tag}`;
    const email = `qa-ds214-plus+${tag}@college.edu`;
    const settings = new SettingsPage(page);
    const modal = settings.addUserModal;
  ...
  test("TC-009: Role defaults to EDITOR when dialog opens", async ({ page }) => {
    const settings = new SettingsPage(page);
    const modal = settings.addUserModal;
